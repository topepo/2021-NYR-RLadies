...
```{r blocks, echo = FALSE, out.width="70%", fig.align='center', fig.alt = "A schematic that shows the formulas or recipes are considered preprocessors. A preprocessor and a model make up a workflow."}
...
```{r faux-model, echo = FALSE, out.width="70%", fig.align='center', fig.alt = "A diagram where PCA is not considered to be an estimation step but the model is classified as such."}
...
```{r real-model, echo = FALSE, out.width="70%", fig.align='center', fig.alt = "A diagram where both PCA and the model are considered to be estimations steps."}
...
.pull-right[
```{r plot-all, out.width = '90%', fig.align='center', dev = 'svg', dev.args = list(bg = "transparent"), echo = FALSE, fig.width=5, fig.height=5, fig.alt = "A plot that shows the rank of each workflow (and submodel within a workflow) versus the estimated RMSE."}
...
```{r plot, out.width = '90%', fig.align='center', dev = 'svg', dev.args = list(bg = "transparent"), echo = FALSE, fig.width=5, fig.height=5, fig.aly = "A plot that shows the rank of each workflow (and no submodels) versus the estimated RMSE."}
...
```
```{r tune, out.width = '70%', fig.align='center', dev = 'svg', dev.args = list(bg = "transparent"), echo = FALSE, fig.width=5, fig.height=5, fig.alt = "A plot of each tuning parameter versus the estimated RMSE."}
updated_res %>% 
  autoplot(id = "filtered_nnet")
```
]
...
.pull-right[
```{r bayes, out.width = '90%', fig.align='center', dev = 'svg', dev.args = list(bg = "transparent"), echo = FALSE, fig.width=5, fig.height=5, cache = TRUE, warning = FALSE, fig.alt = "A plot of the best configuration per workflow  versus the probability of practical equivalence."}
library(tidyposterior)
rmse_res <-
  updated_res %>%
  perf_mod(
...
rmse_res %>% autoplot(type = "ROPE", size = 0.10)
```
]
...
.pull-right[
```{r loss-plot, out.width = '90%', fig.align='center', dev = 'svg', dev.args = list(bg = "transparent"), echo = FALSE, fig.width=5, fig.height=5.5, fig.alt = "A plot of each model term versus the increase in MAE when that term is removed form the model."}
left_join(mae_stats, full_model, by = "id") %>% 
  mutate(loss = .estimate - full_model) %>% 
  filter(wflow_id != "everything_lm") %>% 
  mutate(term = gsub("_lm", "", wflow_id)) %>% 
